...
def get_model(model_name):
    ...
    word_idx = []
    with open(f'../data/{model_name}.json', 'rb') as f:
        for l in f:
...
def get_sequences(abstracts, model_name):
    if model_name == 'training-rnn':
        texts = [format_patent(a) for a in abstracts]
        word_idx, idx_word, num_words, word_counts, new_texts, new_sequences, features, answers = make_sequences(texts, 
                                                                                                                training_length = 50,
                                                                                                                lower = False,
                                                                                                                filters = '!"%;[\\]^_`{|}~\t\n')
    labels = np.zeros((len(features), num_words), dtype = np.int8)
    features = np.array(features)
    for i, ans in enumerate(answers):
        labels[i, ans] = 1
    return features, labels, new_sequences
...
def generate_output(model, sequences, idx_word, training_length = 50, new_words = 50, diversity = 1,
